...
class Params(object):
    """Parameter handler for ancseq command-line interface."""
    ...
    def set_options(self):
        ...
        if self.program_name == "ancseq":
            parser: argparse.ArgumentParser = self.ancseq_options()
        else:
    ...
    def ancseq_options(self):
        """Build and return the ArgumentParser for ancseq."""
        parser = argparse.ArgumentParser(
            description=f"ancseq version {__version__}",
        ...
        parser.add_argument(
            "--fast",
            action="store_true",
            help="Use -fast option in IQ-TREE [FALSE]",
        )
        ...
        parser.add_argument(
            ...
            type=str,
            help=(
                "Specify substitution model for IQ-TREE. IQ-TREE searches the best substitution\n"
        ...
        parser.add_argument(
            "--stop-codon-prob",
            action="store_true",
            help="Stop calculation of codon probabilities in DNA mode [FALSE]",
        )

        parser.add_argument(
            "--asr-only",
            action="store_true",
            help="Skip building tree and reconstruct ancestral states only [FALSE]",
        )
